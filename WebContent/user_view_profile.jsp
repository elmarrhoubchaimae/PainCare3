...
<section id="hero" class="position-relative overflow-hidden  responsive-image">
                ...
                <c:if test="${requestScope.userProfile ne null}">
                    <c:choose>
                        <c:when test="${not empty userProfile.pictureBase64}">
                            <img src="data:image/png;base64,${userProfile.pictureBase64}" alt="User Picture" id="profile-pic" style="margin-right: 50px;"/>
                        </c:when>
                        <c:otherwise>
                            <!-- Affichez une image par défaut ou un message indiquant qu'aucune image n'est disponible -->
                            <img src="${pageContext.request.contextPath}/images/profileDefault.png" alt="Default Profile Picture" id="profile-pic" style="margin-right: 50px;"/>
                        </c:otherwise>
                    </c:choose>
                </c:if>
                <a href="${pageContext.request.contextPath}/userProfile?action=update"><button type="button" class="btn btn-primary ms-md-3" style="box-shadow: 5px 5px 10px rgba(0, 0, 0, 0.1);"> Modifier mon profil </button></a>
            </div>
            <div class="col-md-5">
